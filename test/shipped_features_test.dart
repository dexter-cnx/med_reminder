...
void main() {
  test('registers shipped feature manifests with stable ids and defaults', () {
    ...
    expect(byId.values.every((feature) => feature.manifest.enabledByDefault), isTrue);
    expect(byId['medication']!.manifest.displayNameKey, 'all_meds');
    expect(byId['appointments']!.manifest.displayNameKey, 'appointments');
    expect(byId['emergency']!.manifest.displayNameKey, 'emergency_profile_title');
  });

  test('declares only capabilities currently owned by each shipped feature', () {
    final byId = <String, AppFeature>{
