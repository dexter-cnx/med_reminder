...
void main() {
  test('medication backup DTO round-trips domain data without notification IDs', () {
    final medication = Medication(
      ...
      createdAt: DateTime.utc(2026, 8, 24, 1, 2, 3),
      initialAmount: 20,
      lowThreshold: 5,
    ...
    decoded.fold(
      onSuccess: (value) {
        ...
        expect(value.createdAt.toUtc(), medication.createdAt.toUtc());
  ...
  test('dose log backup DTO round-trips persisted user action', () {
    final log = DoseLog(
      id: 'log-1',
      medId: 'med-1',
      scheduledAt: DateTime.utc(2026, 8, 24, 8),
      takenAt: DateTime.utc(2026, 8, 24, 8, 5),
      status: DoseStatus.taken,
    );

    final decoded = DoseLogBackupDto.decode(DoseLogBackupDto.encode(log));

    decoded.fold(
      onSuccess: (value) {
        expect(value.id, log.id);
        expect(value.medId, log.medId);
        expect(value.scheduledAt.toUtc(), log.scheduledAt.toUtc());
